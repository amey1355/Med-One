import assets from "./assets";

const NFTData = [
  {
    id: "NFT-01",
    category: "Vitamins or Proteins",
    name: "Revital H Health Supplement Capsules Bottle Of 60",
    creator: "REVITAL STORE",
    price: 467.50,
    description:
      "Revital H Health Supplement capsules are nutritional supplements that contain vitamins and 9 minerals as well as Ginseng. These help to get your daily dose of required nutrients and Ginseng helps to aid in mental concentration. Revital H has all the essential nutrients and minerals that you need in one capsule so that you don’t have to worry about taking multiple supplements or tablets. These capsules also help to keep your energy up during the day.",
    image: assets.nft01,
    bids: [
      {
        id: "BID-11",
        name: "Jessica Tan",
        price: 467.25,
        image: assets.person02,
        date: "December 12, 2019 at 12:10 PM",
      },
      {
        id: "BID-12",
        name: "Jennifer Sia",
        price: 467.5,
        image: assets.person03,
        date: "December 27, 2019 at 1:50 PM",
      },
      {
        id: "BID-13",
        name: "Rosie Wong",
        price: 467.75,
        image: assets.person04,
        date: "December 31, 2019 at 3:50 PM",
      },
    ],
  },
  {
    id: "NFT-02",
    category: "pain",
    name: "Vicks Vaporub Xtra Strong",
    creator: "VICKS Store",
    price: 93.06,
    description:
      "Vicks Vaporub Xtra Strong 25 Ml,Relief From Cold, Cough, Headache And Body Pain. It is what it is",
    image: assets.nft02,
    bids: [
      {
        id: "BID-21",
        name: "Jessica Tan",
        price: 7.05,
        image: assets.person04,
        date: "December 12, 2019 at 12:10 PM",
      },
    ],
  },
  {
    id: "NFT-03",
    category: "hair-solutions",
    name: "Indulekha Bringha Hair Oil Bottle Of 50 Ml",
    creator: "Elisabeth aho",
    price: 234,
    description:
      "Indulekha Bringha Oil is an Ayurvedic medicine to prevent hair fall and promote hair growth. It is a 100% natural Ayurvedic oil that has been clinically proven to reduce hair fall and grows new hair in 4 months. It is proven on real male and female consumers having mild to severe hair fall. It is enriched with Bringharaj, a herb known in Ayurveda as 'Keshraj', also known as King of Hair. Extracts from Bringharaj plants are known for their medicinal properties that not only reduce hair fall but also revive & rejuvenate the process of hair growth. The extracts of the herbs are cooked in virgin coconut oil and are made to mature under natural sunlight for 7 days. The power of the sun and the potency of these herbs goes into making every bottle of Indulekha Oil. It's unique and easy to apply straight to the scalp ‘selfie comb’ allows oil to penetrate deep inside the scalp directly, without getting your hands oily. It has a 100% natural fragrance, colour and is recommended by Ayurvedic experts for people suffering from hair fall.",
    image: assets.nft03,
    bids: [
      {
        id: "BID-31",
        name: "Jessica Tan",
        price: 215.25,
        image: assets.person02,
        date: "December 12, 2019 at 12:10 PM",
      },
      {
        id: "BID-32",
        name: "Jennifer Sia",
        price: 200.28,
        image: assets.person03,
        date: "December 27, 2019 at 1:50 PM",
      },
    ],
  },
  {
    id: "NFT-04",
    category: "first-aid",
    name: "Dettol Antiseptic Liquid Bottle Of 550 Ml",
    creator: "DETTOL Store",
    price: 194.6,
    description:
      "Dettol is a brand of cleaning supplies and sanitiser and sterile, presented in 1932 and the producer claimed by the Anglo-Dutch organization Reckitt Benckiser. In Germany, it is sold under the name Sagrotan. Preceding 2002, some Dettol items were marked Detox. Dettol Antiseptic Liquid is a comprehensive use item utilized for security against germs that has been suggested by clinical experts and trusted by moms. Dettol Antiseptic Liquid eliminates germs from skin, shields from contaminations brought about by cuts and scratches and can likewise be utilized as a family sanitiser on home surfaces and in clothing.",
    image: assets.nft04,
    bids: [
      {
        id: "BID-41",
        name: "Jessica Tan",
        price: 190.5,
        image: assets.person02,
        date: "December 12, 2019 at 12:10 PM",
      },
      {
        id: "BID-42",
        name: "Jennifer Sia",
        price: 192.25,
        image: assets.person03,
        date: "December 27, 2019 at 1:50 PM",
      },
      {
        id: "BID-43",
        name: "Rosie Wong",
        price: 190.75,
        image: assets.person04,
        date: "December 31, 2019 at 3:50 PM",
      },
      {
        id: "BID-44",
        name: "Vincent Swift",
        price: 54.15,
        image: assets.person02,
        date: "December 31, 2019 at 3:50 PM",
      },
    ],
  },
  {
    id: "NFT-05",
    category: "first-aid",
    name: "BD Glide with TBL ULTRA-FINE INSULIN SYR U40 31G 6MM 10BG",
    creator: "BD ULTRA FINE Store",
    price: 94,
    description:
      "BD Glide™ with TBL U40 Insulin Syringe BD Ultra-Fine™ Needle Insulin Syringe (6mm, 31G) comes with TBL needle technology for a smooth and comfortable injection experience. A thin wall allows thinner needles with better drug flow. The tri-bevel tip allows easy penetration into the skin. BD Glide™ with TBL U40 Insulin Syringe BD Ultra-Fine™ Needle Insulin Syringe (6mm, 31G) has a 6mm needle in line with the latest clinical evidence and injection technique recommendations which advocate the use of a 6-mm syringe needle for insulin injection*^.",
    image: assets.nft05,
    bids: [
      {
        id: "BID-51",
        name: "Jessica Tan",
        price: 85,
        image: assets.person02,
        date: "December 12, 2019 at 12:10 PM",
      },
    ],
  },
  {
    id: "NFT-06",
    category: "Vitamins or Proteins",
    name: "Ensure High Protein Nutrition For Active Adults (Chocolate Flavour), 400g",
    creator: "ENSURE Store",
    price: 609,
    description:
      "Ensure high protein is scientifically formulated for active adults. It has high protein and L carnitine to help support strength and energy metabolism. it has a perfect blend of 31 vital nutrients. It is suitable for both males and females, active adults it contains high-quality proteins that supports muscle health. Scientifically formulated with 31 nutrients including protein, calcium, vitamin D that could be missing from your diet. it is great to taste, and it contains high quality nutrients which helps give you strength to keep doing things you enjoy. This specialized nutritional drink is available in chocolate and vanilla flavour and comes in 200gm and 400gm size.",
    image: assets.nft06,
    bids: [
      {
        id: "BID-61",
        name: "Jessica Tan",
        price: 655,
        image: assets.person02,
        date: "December 12, 2019 at 12:10 PM",
      },
      {
        id: "BID-62",
        name: "Jennifer Sia",
        price: 650,
        image: assets.person03,
        date: "December 27, 2019 at 1:50 PM",
      },
      {
        id: "BID-63",
        name: "Rosie Wong",
        price: 615.50,
        image: assets.person04,
        date: "December 31, 2019 at 3:50 PM",
      },
      {
        id: "BID-64",
        name: "Siti Nurhaliza",
        price: 21.25,
        image: assets.person02,
        date: "December 31, 2019 at 3:50 PM",
      },
      {
        id: "BID-65",
        name: "Kaitlyn Lee",
        price: 7.25,
        image: assets.person02,
        date: "December 31, 2019 at 3:50 PM",
      },
    ],
  },
  {
    id: "NFT-07",
    category: "hair-solutions",
    name: "Scalpe Plus Anti Dandruff Shampoo Bottle Of 75 Ml",
    creator: "SCALPE Store",
    price: 210.6,
    description:
      "Scalpe Plus Shampoo is a proven shampoo for the treatment of dandruff-related issues. The ingredients are based on the ketoconazole + ZPTO formula, which is renowned and recognized across the globe for its effectiveness against dandruff. Ketoconazole works by fighting against the primary cause of dandruff in humans, the fungus Malassezia. By destroying this fungus and with regular use even eliminating it from the scalp, Scalpe Shampoo can provide long-term relief from dandruff. Regular use of Scalpe is also great for your hair as the shampoo base conditions your hair making it softer. From the IMS Prescription Audit MAT (Mar 20) Scalpe Plus was rated as the most prescribed anti-dandruff shampoo currently on the market, especially due to its ability to fight off recurring dandruff.",
    image: assets.nft07,
    bids: [],
  },
];

export { NFTData };
